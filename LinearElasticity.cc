...
PetscErrorCode LinearElasticity::SetUpLoadAndBC(TopOpt *opt){
	...
	for (PetscInt i=0;i<nn;i++){
		...
		if (i % 3 == 0 && PetscAbsScalar(lcoorp[i]-opt->xc[0]) < epsi && 
			...
			VecSetValueLocal(RHS,i+2,-0.01,INSERT_VALUES);
		}
		...
		if (i % 3 == 0 && PetscAbsScalar(lcoorp[i]-opt->xc[0]) < epsi && 
				  ...
				  PetscAbsScalar(lcoorp[i+2]-opt->xc[5]) < epsi ){
			VecSetValueLocal(RHS,i+2,-0.005,INSERT_VALUES);
		}
		if (i % 3 == 0 && PetscAbsScalar(lcoorp[i]-opt->xc[0]) < epsi && 
...
PetscErrorCode LinearElasticity::ComputeObjectiveConstraintsSensitivities(TopOpt *opt) {
	...
	// Robust formulation ==============
	PetscScalar *xpEro;
	VecGetArray(opt->xPhysEro,&xpEro);
	PetscScalar *dfEro;
	VecGetArray(opt->dfdx,&dfEro);
	// =================================
	...
	for (PetscInt i=0;i<nel;i++){
		...
		*/
		...
		// Robust formulation ====================================================================
		// Add to objective
		opt->fx += (opt->Emin + PetscPowScalar(xpEro[i],opt->penal)*(opt->Emax - opt->Emin))*uKu;
		// Set the Senstivity
		dfEro[i]= -1.0 * opt->penal*PetscPowScalar(xpEro[i],opt->penal-1)*(opt->Emax - opt->Emin)*uKu;
		// ========================================================================================
	}
	...
	*/
	...
	// Robust formulation =========================
	VecRestoreArray(opt->xPhysEro,&xpEro);
	VecRestoreArray(opt->dfdx,&dfEro);
	// ===========================================
