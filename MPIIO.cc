...
PetscErrorCode MPIIO::WriteVTK(DM da_nodes, Vec U, TopOpt *opt, PetscInt itr){
  ...
  // Here we only have one "timestep" (no optimization)
	...
	// CELL FIELD(S)
	PetscScalar *xpp, *xp, *tf, *xS, *xV, *xEro, *xDil, *gi, *giDIL;
	VecGetArray(opt->x,&xp);
	VecGetArray(opt->xPhys,&xpp);
	VecGetArray(opt->TestFilter,&tf);
	VecGetArray(opt->xSolid0,&xS);
	VecGetArray(opt->xSolid1,&xV);
	VecGetArray(opt->xPhysEro,&xEro);
	VecGetArray(opt->xPhysDil,&xDil);
	VecGetArray(opt->gms,&gi);
	VecGetArray(opt->gmsDIL,&giDIL);
	...
	for (unsigned long int i=0; i<nCellsMyrank[0]; i++){
		...
		workCellField[i+2*nCellsMyrank[0]] = float(tf[i]);
		workCellField[i+3*nCellsMyrank[0]] = float(xS[i]);
		workCellField[i+4*nCellsMyrank[0]] = float(xV[i]);
		workCellField[i+5*nCellsMyrank[0]] = float(xEro[i]);
		workCellField[i+6*nCellsMyrank[0]] = float(xDil[i]);
		workCellField[i+7*nCellsMyrank[0]] = float(gi[i]);
		workCellField[i+8*nCellsMyrank[0]] = float(giDIL[i]);
	}
	...
	VecRestoreArray(opt->TestFilter,&tf);
	VecRestoreArray(opt->xSolid0,&xS);
	VecRestoreArray(opt->xSolid1,&xV);
	VecRestoreArray(opt->xPhysEro,&xEro);
	VecRestoreArray(opt->xPhysDil,&xDil);
	VecRestoreArray(opt->gms,&gi);
	VecRestoreArray(opt->gmsDIL,&giDIL);
